...
router.post('/', CheckLogin, CreateTaskValidator, validatedResult, async function (req, res, next) {
    try {
        let { title, description, priority, project_id, assignee_name, deadline } = req.body;
        ...
        // Nếu có assignee_name, tìm user ID từ username
        let assignee_id = null;
        if (assignee_name) {
            let userController = require('../controllers/users');
            let assignee = await userController.GetAnUserByUsername(assignee_name);
            if (!assignee) {
        ...
        // Socket: thông báo task mới cho project room
        sendNewTask(io, project_id, task);

        // Lưu DB notification + activity log
        await logController.CreateLog(req.user.id, 'create_task', 'task', task.id,
            `Tao task: ${title}`, { project_id });

        // Socket + DB notification cho người được giao task
        if (assignee_id && assignee_id != req.user.id) {
            ...
            await notifController.CreateNotification(
                assignee_id, 'task_assigned',
                'Task moi duoc giao',
                `Ban duoc giao task: "${title}"`,
                'task', task.id
            );
        }
...
router.put('/:id', CheckLogin, async function (req, res, next) {
    try {
        ...
        if (updateData.assignee_name) {
            let userController = require('../controllers/users');
            let assignee = await userController.GetAnUserByUsername(updateData.assignee_name);
            if (!assignee) {
                res.status(404).send({ message: 'user khong ton tai' });
                return;
            }
        ...
        let updated = await taskController.UpdateATask(req.params.id, updateData);

        // Log hoạt động
        await logController.CreateLog(req.user.id, 'update_task', 'task', req.params.id,
            `Cap nhat task: ${updated.title}`, updateData);

        // Socket: thông báo update cho project room
        sendTaskUpdate(io, task.project_id, updated);

        // Socket + DB notification nếu đổi assignee
        if (updateData.assignee_id && updateData.assignee_id != task.assignee_id) {
            sendNotification(io, updateData.assignee_id, {
                type: 'task_assigned',
                message: `Ban duoc giao task: "${updated.title}"`,
                task: updated
            });
            await notifController.CreateNotification(
                updateData.assignee_id, 'task_assigned',
                'Task moi duoc giao',
                `Ban duoc giao task: "${updated.title}"`,
                'task', updated.id
            );
        }
...
router.patch('/:id/status', CheckLogin, async function (req, res, next) {
    try {
        ...
        // Log hoạt động
        await logController.CreateLog(req.user.id, 'update_status', 'task', req.params.id,
            `Cap nhat trang thai: ${task.title} -> ${status}`,
        ...
        // Socket + DB notification cho creator nếu assignee đổi sang done/review
        if (task.creator_id !== req.user.id && (status === 'done' || status === 'review')) {
            ...
            await notifController.CreateNotification(
                task.creator_id, 'task_status_changed',
                'Cap nhat trang thai task',
                `Task "${task.title}" da chuyen sang: ${status}`,
                'task', task.id
            );
        }
...
// POST /api/v1/tasks/:id/comments - Thêm comment + socket real-time
router.post('/:id/comments', CheckLogin, CreateCommentValidator, validatedResult, async function (req, res, next) {
    try {
        ...
        // Log hoạt động
        await logController.CreateLog(req.user.id, 'add_comment', 'task', req.params.id,
            `Them comment vao task: ${task.title}`);

        // Socket: gửi comment real-time
        sendNewComment(io, req.params.id, comment);

        // Socket + DB notification cho task owner
        if (task.creator_id !== req.user.id) {
            ...
            await notifController.CreateNotification(
                task.creator_id, 'new_comment',
                'Binh luan moi',
                `${req.user.username} da binh luan vao task: "${task.title}"`,
                'task', req.params.id
            );
        }
